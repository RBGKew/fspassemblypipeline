...
def parse_genomescope_summary(summary_file: Path) -> dict[str, str]:
...
def parse_genomescope_log(log_file: Path) -> dict[int, dict[str, str]]:
...
def collect_peak_stats(summary_file: Path, log_peak_stats: dict[str, str]) -> dict[str, str]:
    summary_stats = parse_genomescope_summary(summary_file)
    return {
...
def collect_rows(input_dir: Path, peak_calls: dict[str, dict[str, str]]) -> list[dict[str, str]]:
    ...
    for sample_dir in sorted(path for path in input_dir.iterdir() if path.is_dir()):
        ...
        genomescope_log_file = sample_dir / f"{sample_id}.genomescope.log"
        if not hist_file.exists():
            continue

        unique_kmers = parse_unique_kmers_from_hist(hist_file)
        total_kmers = parse_total_kmers_from_fastk_log(fastk_log_file)
        genomescope_log_stats = parse_genomescope_log(genomescope_log_file)

        peak1 = collect_peak_stats(sample_dir / "peak_1" / "summary.txt", genomescope_log_stats.get(1, {}))
        peak2 = collect_peak_stats(sample_dir / "peak_2" / "summary.txt", genomescope_log_stats.get(2, {}))
